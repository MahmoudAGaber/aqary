...
import 'Widgets/EstateDetails.dart';

class UpdateContract extends ConsumerStatefulWidget {
  String contractId;
  UpdateContract({super.key,required this.contractId});

  @override
  ConsumerState<UpdateContract> createState() => _RentState();
}

class _RentState extends ConsumerState<UpdateContract> {
  ...
  void initState() {
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async{
      ...
      ref.read(ContractMineProvider.notifier).getOneContract(widget.contractId).then((value)async{
        ...
        var fromDate = DateConverter.convertStringToDatetime(value.to).subtract(Duration(days: 1));
        toDateEditingController.text = DateConverter.isoStringToLocalDateOnly2(DateConverter.localDateToIsoString(fromDate));
                                      ...
                                      if (_formKey.currentState!.validate()) {
                                        ...
                                        await fetchImageAsBytes(contract.data!.first.renter_signature).then((renterSignature)async{
                                          File file =  await PdfInvoiceApi.generate(
                                              ...
                                              paymentStyle: PaymentHelper.getArabicPaymentStyle(paymentSystem)).then((file)async{

                                            File ownerSignatureFile = await writeToTempFile(ref.watch(signatureProvider).data!,'ownerSignature');

                                            ref.read(ContractMineProvider.notifier).acceptContract(ownerSignatureFile.path,widget.contractId,file.path);
                                            return file;
                                          });
                                        });
                                      }
                                      Navigator.push(context, MaterialPageRoute(builder: (contract)=>Homepage(page: 0)));
                                    }else{
                                      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("من فضلك قم بإضة التوقيع الخاص بك",style: TextStyle(color: Colors.black),)));
                                    }
                                  }
